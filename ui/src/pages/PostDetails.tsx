...
const PostDetails: React.FC = () => {
    const { id, post_id: postId } = useParams();
    const userState = useAppSelector(state => state.userSlice.user);

    const [showComments, setShowComments] = useState<boolean>(false);

    const navigate = useNavigate();
    const { data: post, isFetching: postFetching, isLoading: postLoading } = useGetPostQuery(parseInt(postId!));
    const {
        data: comments,
            ...
            <Container>
                <IconWrapper ref={iconWrapperRef}>
                    <AbsoluteIconButton
                        icon={faReply}
                        $left
                        $containerWidth={iconWrapperRef.current?.clientWidth}
                        onClick={() => navigate(-1)}
                    />
                </IconWrapper>
                <Wrapper>
